...
def build_constraint_table(constraints, agent):
    ...
    constraint_list = [
        constraint for constraint in constraints if constraint['agent'] == agent]
    neg_constraint_table = dict()
    pos_constraint_table = dict()
    for constraint in constraint_list:
        loc = constraint['loc'][0] if len(
            constraint['loc']) == 1 else tuple(constraint['loc'])
...
def a_star(my_map, start_loc, goal_loc, h_values, agent, constraints):
    ...
    """
    my_map_size = [cell for submap in my_map for cell in submap].count(
        False)  # count moveable spaces
    neg_constraint_table, pos_constraint_table = build_constraint_table(
        constraints, agent)
    ...
    # Hash node with its corresponding timestamp
    ...
    while open_list:
        ...
        for direction in range(5):
            ...
            if next_t in pos_constraint_table:
                ...
                constraints[1] = ((cur_node['loc'], next_loc),
                                  next_t) in pos_constraint_table
...
def build_mdd(my_map, max_cost, cost_offset, start_h_values, goal_h_values):
    ...
    """
    ict = set()
    valid_loc = [(v, h) for v, h in start_h_values.items()
                 if h + goal_h_values[v] < max_cost]
    for t in range(max_cost - 1):
        possible_loc = [v for v, h in valid_loc if h <=
                        t and t + goal_h_values[v] < max_cost]
